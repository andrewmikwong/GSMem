...
#define SAMPLE_PERIOD 215
#define TS_RATIO (TRANSMISSION_PERIOD/SAMPLE_PERIOD)
#define BUF_SIZE (2*(TRANSMISSION_PERIOD/SAMPLE_PERIOD))
...
//min=starting arfcn, freq_ul is the frequency at min
static struct band {
...
uint8_t file[4096];
int file_index=0;
int16_t ringbuf[BUF_SIZE];
uint16_t head=-1;
int16_t thresh=-96;
int synced=0;
int counter=0;
//true if looking for preamble
int preamble=1;

//only works for values >-bsize. c's modulus is different from python's
int16_t modulo(int16_t a, int bsize){
	if (a<0){
		a+=bsize;
	}
	return a%bsize;
...
void buf_push(int16_t power_meas){
	head=(head+1)%BUF_SIZE;
	ringbuf[head]=power_meas;
	//print_ringbuf();
}

int ringbuf_high_low(){
	int i=(head+1)%BUF_SIZE;
	while (i!=modulo(head-BUF_SIZE/2+1,BUF_SIZE)){
		if (ringbuf[i]<=thresh){
			return 0;
		}
		i=(i+1)%BUF_SIZE;
	}
	while (i!=(head+1)%BUF_SIZE){
		if(ringbuf[i]>=thresh){
			return 0;
		}
		i=(i+1)%BUF_SIZE;
...
void add_bit(int bit){
	if (cur_bit==8){
		printf("writing a byte: \n");
		file[file_index++]=cur_char;
		cur_char=0;
		cur_bit=0;
		transmite_file();
	}
	cur_char|=bit<<cur_bit;
	++cur_bit;	
}

//returns average dbm of last TRANSMISSION_PERIOD/SAMPLE_PERIOD samples
int16_t get_avg(){
	int i=modulo(head-BUF_SIZE/2+1, BUF_SIZE);
	int16_t avg=0;
	while (i!=(head+1)%BUF_SIZE){
		avg+=ringbuf[i];
		i=(i+1)%BUF_SIZE;
	}
	return avg/(TRANSMISSION_PERIOD/SAMPLE_PERIOD);
}

//return true if phone is too slow
int clock_skewed(){
	int avg=0;
	
}

//call back to our rootkit
void sample_dsp(void *p){
	osmo_timer_schedule((struct osmo_timer_list*)p, SAMPLE_PERIOD);
	printf("%d\n", dbm);
	buf_push(dbm);
	
	if(!synced){
		if (check_buf_sync()){
		//expect next value to be opposite of current 
			puts("SYNCHRONIZED!\n");
			synced=1;
			add_bit(1);
		}
...
*/
		if(counter==TRANSMISSION_PERIOD/SAMPLE_PERIOD-1){
			if(get_avg()<thresh){
				add_bit(0);
				if(ringbuf[head]>=thresh){
					counter=(counter+1)%(TRANSMISSION_PERIOD/SAMPLE_PERIOD);
				}
			} else{
				add_bit(1);
				if(ringbuf[head]<thresh){
...
static struct osmo_timer_list sample_dsp_timer = {
	.cb = &sample_dsp,
	.data = &sample_dsp_timer
} ;
...
//	display_unset_attr(DISP_ATTR_INVERT);
	...
	osmo_timer_schedule(&sample_dsp_timer, SAMPLE_PERIOD);
	arfcn=207;
	init_ringbuf();
	init_file();
